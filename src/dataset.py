...
class DocRelationDataset:
    ...
    def __call__(self):
        ...
        for pmid in pbar:
            ...
            # TEXT
            doc = []
            # for sentence in self.loader.documents[pmid]:  # GDA dataset
            #     sent = []
            #     if self.data_type == 'train':
            #         for w, word in enumerate(sentence):
            #             if (word in self.mappings.singletons) and (random.uniform(0, 1) < 0.5):
            ...
            #     doc += [sent]
            for sentence in self.loader.documents[pmid]:  # CDR dataset
                word2id = json.load(open(os.path.join("../embeds/word2id.json")))
                sent = []
                for w, word in enumerate(sentence):
                    word = word.lower()
                    if word in word2id:
            ...
            position = np.zeros_like(words)
            entity_type = np.zeros_like(words)
            nodes = []
            ent = []
            ...
            for id_, (e, i) in enumerate(self.loader.entities[pmid].items()):
                for sent_id, m1, m2 in zip(i.sentNo.split(':'), i.mstart.split(':'), i.mend.split(':')):
                    ...
                    position[int(m1): int(m2)] = id_ + 1
                    entity_type[int(m1): int(m2)] = self.mappings.type2index[i.type]
            ...
            if (trel == -1).all():  # no relations --> ignore
                continue
            temp_ = np.array([len(s) for s in doc])
            self.data += [{'ents': ent, 'text': doc, 'dist': dist,
                           'rels': trel, 'info': rel_info,
                           'position': position, 'entity_type': entity_type,
                           'adjacency': adjacency,
                           'section': np.array([len(self.loader.entities[pmid].items()), ent.shape[0], len(doc)], 'i'),
                           'word_sec': temp_, 'doc_sec': np.array(temp_).sum(),
                           'words': words}]
